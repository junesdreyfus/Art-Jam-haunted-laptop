...
//might add third version with arrays if mathilde pulls through//
...
//third time's the charm/
  ...
  let x3= constrain (mouseX+25, screen.x+25, screen.w+screen.x);
...
let g= map(mouseX, screen.x, screen.x+screen.w, 1, 5);
  ...
  //ghost will first hide behind wall geometry/
  push();
  fill(ghost.r, ghost.g, ghost.b),   
  //creepy unreleased desk/
  //rect(2.5*screen.w/4, screen.h/2, 150, ) 
  ellipse(ghost.x, ghost.y, ghost.w, ghost.h),
  ellipse(ghost.x, ghost.y, ghost.w2, ghost.h2),
  pop();
  ...
  //second smaller ghost because I can't make the first one move exponentially/
    push();
  fill(ghost.r, ghost.g, ghost.b),   
  ellipse(ghost.x, ghost.y, ghost.w2, ghost.h2),
  pop();
